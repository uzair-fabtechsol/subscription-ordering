...
const validateUserWithRole = async (
  ...
  const user = await UserModel.findById(userId).select(
    "userType status firstName lastName email"
  );
  if (!user) throw new AppError(`${label} not found`, 404);
  if (user.userType !== role)
    throw new AppError(`${label} must be a ${role}`, 400);
  if (user.status !== UserStatus.ACTIVE)
    throw new AppError(`${label} is not active`, 400);
  return user;
};
...
const ensureDeliveryFields = (
  ...
  if (
    ...
    !allowedIntervals.includes(deliveryInterval)
  )
